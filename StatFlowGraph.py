...
f1temps = [f1['flows'][i]['temp'][0] for i in list(f1['flows'].keys())]
f2temps = [f2['flows'][i]['temp'][0] for i in list(f2['flows'].keys())]
f3temps = [f3['flows'][i]['temp'][0] for i in list(f3['flows'].keys())]
f4temps = [f4['flows'][i]['temp'][0] for i in list(f4['flows'].keys())]
f1temps.sort()
f2temps.sort()
f3temps.sort()
f4temps.sort()
...
im = ax[1][0].imshow(data3, cmap=cmap, norm=norm)
im = ax[1][1].imshow(data4, cmap=cmap, norm=norm)

minFlowshow1 = 1e-3
f1Sums = loadFile(SummedFlowsPath, "SummedFlowsF1.dat")[0]
minFlowshow1 = graphSummedFlow(f1Sums, xrange, yrange, fig, ax[0][0], minFlowshow1, data3, cbar=(minFlowshow1, 1e-2))
minFlowshow2 = 1e-3
f2Sums = loadFile(SummedFlowsPath, "SummedFlowsF2.dat")[0]
minFlowshow2 = graphSummedFlow(f2Sums, xrange, yrange, fig, ax[0][1], minFlowshow2, data4, cbar=(minFlowshow2, 1e-2))

minFlowshow3 = 1e-3
f3Sums = loadFile(SummedFlowsPath, "SummedFlowsF3.dat")[0]
minFlowshow3 = graphSummedFlow(f3Sums, xrange, yrange, fig, ax[1][0], minFlowshow3, data3, cbar=(minFlowshow3, 1e-2))
minFlowshow4 = 1e-3
f4Sums = loadFile(SummedFlowsPath, "SummedFlowsF4.dat")[0]
minFlowshow4 = graphSummedFlow(f4Sums, xrange, yrange, fig, ax[1][1], minFlowshow4, data4, cbar=(minFlowshow4, 1e-2))
...
ax[0][0].set_title("Trajectory 1 @ " + "Total Flows" + "  MinFlowShow = " + str(np.round(minFlowshow1, abs(int(np.log10(minFlowshow1) - 2)))))
ax[0][1].set_title("Trajectory 2 @ " + "Total Flows" + "  MinFlowShow = " + str(np.round(minFlowshow2, abs(int(np.log10(minFlowshow2) - 2)))))
ax[1][0].set_title("Trajectory 3 @ " + "Total Flows" + "  MinFlowShow = " + str(np.round(minFlowshow3, abs(int(np.log10(minFlowshow3) - 2)))))
ax[1][1].set_title("Trajectory 4 @ " + "Total Flows" + "  MinFlowShow = " + str(np.round(minFlowshow4, abs(int(np.log10(minFlowshow4) - 2)))))
...
plt.savefig(("/home/thana1dr/Research_Results&Data/Week8/Plots/SummedFlows/4Traj005.png"), dpi=500)

print("Runtime: " + str((time.time()-beginTrue)*1000) + " ms\n")
plt.show()
plt.close()